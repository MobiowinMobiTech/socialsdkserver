			...
			while (true)
			{
				...
				if (!reqJson.isNull(ApplicationConstants.FbApiKeys.DATA))
				{
					...
					if (arrFriendsList.length() > 0)
					{
						...
						for (int i = 0; i < arrFriendsList.length(); i++)
						{
							...
							JSONObject objFriend = (JSONObject) arrFriendsList
									.get(i);
							JSONObject objPicture = objFriend
									.getJSONObject(ApplicationConstants.FbApiKeys.PICTURE);
							JSONObject objData = objPicture
									.getJSONObject(ApplicationConstants.FbApiKeys.DATA);
				...
				if (!reqJson.isNull("paging"))
				{
					...
					if (!paging.isNull("next"))
					{
						...
						reqJson = new JSONObject(customHTTPConnector(paging
								.get("next").toString()));
					} else
					{
						break;
